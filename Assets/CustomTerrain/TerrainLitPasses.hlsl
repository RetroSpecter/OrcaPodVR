...
half4 SplatmapFragment(Varyings IN) : SV_TARGET
{
	...
	// caustics code
	float2 distortion_noise_uv = TRANSFORM_TEX(IN.uvMainAndLM.xy, _NoiseTex) + _Time * 0.0001f * _DistortionSpeed;
	half4 distortion_noise = SAMPLE_TEXTURE2D(_NoiseTex, sampler_NoiseTex, distortion_noise_uv);

	float2 fade_noise_uv = TRANSFORM_TEX(IN.uvMainAndLM.xy, _NoiseTex) * 0.5f + _Time * 0.1f * _FadeSpeed;
	half4 fade_noise = SAMPLE_TEXTURE2D(_NoiseTex, sampler_NoiseTex, fade_noise_uv);

	float2 output_uv = TRANSFORM_TEX(IN.uvMainAndLM.xy, _CausticTex) + _Time * _TexSpeed + distortion_noise * _TexDistortion;
	half4 caustic = SAMPLE_TEXTURE2D(_CausticTex, sampler_CausticTex, output_uv);
	caustic *= _CausticOpacity * fade_noise;
	color += caustic;
